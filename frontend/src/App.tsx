...
import { getSampleData, calculateMetrics, getRankedKeywords, exportToCSV } from './services/api';
...
function App() {
  ...
  const handleFetchData = async (config: {
    ...
    try {
      ...
      const { results } = await getRankedKeywords(
        config.domain,
        config.locationCode,
        config.languageCode,
        100,
      ...
      // Calculate with the new data (keep sample brand keywords for now)
      const calcResults = await calculateMetrics(brandKeywords, results);
